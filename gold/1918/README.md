...
<p align=center>
    <image src='./postfix.avif'>
</p>
<center>결과: ABC*+DE/-</center>
<br>
...
## 문제풀이
스택을 이용하여 구현하였다. 일단 알파벳은 그대로 출력하기 위해 `res`에 넣었다.

연산자 같은 경우에는 곱셈 나눗셈이 우선도가 더 높기 때문에 `prior`에 지정하였고 이를 바탕으로 스택의 top의 우선도가 더 낮아질 때까지 pop한 값을 `res`에 저장하였다. 그 뒤에 현재 연산자를 스택에 저장하였다.

괄호와 같은 경우에는 여는 괄호는 스택에 저장하고 닫는 괄호를 만나면 여는 괄호가 나올 때까지 `res`에 저장하였다.

전부 읽은 뒤에는 스택에 남은 연산자를 pop하면서 `res`에 저장하였고 이 결과를 join으로 결합하여 출력하였다.