...
## 입력
입력은 여러 개의 테스트 케이스로 이루어져 있다. 첫째 줄에는 테스트 케이스의 개수 C가 주어진다. 각각의 케이스는 11줄로 이루어져 있고, i번 줄은 0과 100사이의 11개의 정수 s<sub>ij</sub>를 포함하고 있다. s<sub>ij</sub>는 i번선수가 j번 포지션에서 뛸 때의 능력이다. 모든 선수에게 적합한 포지션의 수는 최대 5개이다. (능력치가 0보다 크다)

## 출력
각각의 테스트 케이스에 대해서, 모든 포지션의 선수를 채웠을 때, 능력치의 합의 최댓값을 한 줄에 하나씩 출력한다. 항상 하나 이상의 올바른 라인업을 만들 수 있다.

## 문제풀이
백트래킹을 사용한 깊이 우선 탐색으로 문제를 해결했다.

값이 하나의 세로줄에서 하나의 값만 가져올 수 있게 `visited`를 통해 구현하였다. 그리고 백트래킹은 각 세로줄의 최대값들을 모아서 현재까지 저장한 값들 + 아직 고르지 않은 세로줄의 최대값들의 합이 기존의 저장한 값보다 작으면 더이상 진행하지 않게 `return`하였다.