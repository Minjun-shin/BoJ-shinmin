...
## 문제풀이
가장 밑의 주사위에서 하나씩 위로 쌓으며 진행했다.

현재 주사위에서 주어진 숫자가 어디에 위치하는지를 `index`메서드를 사용해서 구하고 이떄 나온 수를 통해 반대쪽 수를 구하고 나머지 수 중 최대값을 `total`값에 더해 주었다. 이를 마지막 주사위까지 진행하고 이렇게 나온 `total`값 중 최대값을 출력하였다.